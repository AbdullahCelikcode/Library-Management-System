...
public class WriterManager implements WriterService {
    ...
    public void add(CreateWriterRequest CreateWriterRequest) {
        Writer writer = this.modelMapperService.forRequest().map(CreateWriterRequest, Writer.class);
    ...
    public List<GetAllWriterResponse> findAll() {
        ...
        return writers.stream().map(writer ->
                this.modelMapperService.forResponse().map(writer, GetAllWriterResponse.class)).collect(Collectors.toList());
    }
    ...
    public GetByIdWriterResponse findById(int id) {
        ...
        return this.modelMapperService.forResponse().map(writer, GetByIdWriterResponse.class);
    }

    @Override
    public void update(UpdateWriterRequest updateWriterRequest) {
        Writer writer = this.modelMapperService.forRequest().map(updateWriterRequest, Writer.class);
